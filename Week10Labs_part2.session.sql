...
-- @block
INSERT INTO `Customer2` (`cus_code`, `cus_lname`, `cus_fname`, `cus_initial`, `cus_areacode`, `cus_phone`, `cus_balance`) VALUES
(10010, 'Ramas', 'Alfred', 'A', '615', '844-2573', 0.0000);
...
-- @block
SELECT cus_code, CONCAT(cus_fname, " ", cus_lname) AS Name FROM customer;

-- @block
SELECT * FROM invoice WHERE cus_code = 10011;

-- @block
SELECT inv_number FROM invoice WHERE inv_date <= '2004-01-16';

-- @block
SELECT p_code, p_descript, p_price FROM product WHERE p_price BETWEEN 15 AND 25;

-- @block
SELECT MIN(p_price) FROM product;

-- @block
SELECT * FROM customer WHERE cus_fname LIKE 'A%';

-- @block
SELECT invoice.inv_number, invoice.inv_date, invoice.cus_code, CONCAT(customer.cus_fname, " ", cus_lname) AS Name FROM invoice JOIN  customer ON invoice.cus_code = customer.cus_code;

-- @block
SELECT cus_code, cus_fname, cus_lname FROM customer ORDER BY cus_lname DESC;

-- @block
SELECT cus_code, cus_fname, cus_lname FROM customer ORDER BY cus_lname ASC;

-- @block
SELECT inv_number, line_number, line_units, line_price, line_units * line_price AS line_total FROM line WHERE inv_number = 1003;

-- @block
SELECT SUM(line_units * line_price) AS total_sales FROM line;

-- @block
SELECT AVG(line_units * line_price) AS average_sales FROM line;

-- @block
SELECT inv_number, SUM(line_units * line_price) AS invoice_total FROM line GROUP BY inv_number ORDER BY inv_number;